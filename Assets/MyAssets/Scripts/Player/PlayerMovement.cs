...
public class PlayerMovement : MonoBehaviour
{
    ...
    [SerializeField]
    [Tooltip("Control speed to face a movement direction")]
    [Range(0.0f, 0.3f)] float rotationSmooth = 0.05f;

    [Space(10)]
    [Header("CrossHair")]
    [Tooltip("Player FPS Camera center")] [SerializeField] private Transform playerFpsCameraCenter;
    ...
    [SerializeField]
    [Tooltip("The character uses its own gravity value with RB. Engine default is -9.81f")]
    float gravity = -9.81f;
    ...
    [Space(10)]
    [Header("Player is grounded")]
    [Tooltip("Scans the assignated layer in order to create the jump action")]
    ...
    void GunFollowCam()
    { //set the gun at same point that camera
        GunPositionController gunPositionScript = GetComponent<GunPositionController>();
        // Verifica si el script gunposition está presente
        if (gunPositionScript != null)
        {
            // Llama a la función que ajusta la rotación del arma
            gunPositionScript.GunPosition();
        }
    }
        #endregion
    }
